...
def text_to_speech(text: str):
    """Converts text to speech using kokoro TTS and waits for playback."""
        ...
        for _, _, audio in tts_pipeline(text, voice=KOKORO_VOICE):
            ...
            if audio is not None:
                ...
                if isinstance(audio_np, np.ndarray):
                    ...
                    logging.warning(
                        "Audio data is not a numpy array after conversion. Skipping chunk.")
...
async def send_tts_response(websocket: WebSocket, text_response: str):
    ...
    logging.info(f"Sending TTS response: '{text_response}'")
    try:
        ...
        for _, _, audio in audio_stream:
            if audio is not None:
                ...
                with io.BytesIO() as wav_io:
                    sf.write(wav_io, audio_np, KOKORO_SAMPLERATE, format='WAV')
                    wav_io.seek(0)
                    wav_bytes = wav_io.read()
                    # Stream TTS audio over websocket
                    await websocket.send_bytes(wav_bytes)
        logging.info("TTS response sent.")
